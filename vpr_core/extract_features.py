...
def main(
    ...
    if len(dataset.names) == 0:
        ...
        return feature_path
    cprint(f"    ├─ Dataset prep: {time.time() - t_dataset:.3f}s", 'cyan')
    ...
    cprint(f"    ├─ Model loading: {time.time() - t_model:.3f}s", 'cyan')
        ...
        with h5py.File(str(feature_path), "a", libver="latest") as fd:
            try:
                ...
                if "No space left on device" in error.args[0]:
                    ...
                    del grp, fd[name]
                raise error
        t_h5_write_total += time.time() - t_h5

        del pred

    cprint(f"    ├─ Inference: {t_inference_total:.3f}s", 'cyan')
    cprint(f"    ├─ H5 writing: {t_h5_write_total:.3f}s", 'cyan')
    cprint(f"    └─ Total extract_features.main: {time.time() - t_start:.3f}s", 'yellow')
